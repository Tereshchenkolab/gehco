...
fluidPage(theme = shinytheme("flatly"), #selecting a flat and modern theme for the app display
          
          titlePanel('Predicting 3-year competing risk of monomorphic ventricular tachycardia (MMVT) and polymorphic
                     ventricular tachycardia/ventricular fibrillation (PVT/VF)'), #title of the app
          hr(),  #horizontal rule
          
          sidebarLayout(
            sidebarPanel(id = "sidebar",
                         h5("Demographic Characteristics"),  #displays the side title header
                         textInput('agey', 'Age (Years)',placeholder = '18 - 100'), #displays the placeholder as range in the app
                         selectInput('sex', 'Gender', choices = c('Female', 'Male')), #displays the choices in the app
                         ...
                         #condition table display on the side panel
                         ...
                         textInput('vt_zone_bpm', 
                                   label = HTML('<span>VT zone cut-off (bpm) <br> <span style="font-size: smaller;">(If VT zone is OFF, enter VF zone value here)</span></span>'), 
                                   placeholder = '105 - 190'),
                         # textInput('vt_zone_bpm', 'VT zone cut-off (bpm)',placeholder = '105 - 190'),
                         # HTML('<span style="font-size: smaller;">(If VT zone is OFF, enter VF zone value here)</span>'),
            ...
            mainPanel(                       #displays the buttons in the main panel
              ...
              DT::dataTableOutput('result'), #passes on the result in the DT table from server side to ui side to display
              br(),
